...
### Set up
First we need to fork our reference application repo (e.g. using [hub](https://hub.github.com/)):

    $ git clone https://github.com/ministryofjustice/cloud-platform-reference-app.git
    $ hub fork
    $ git remote -v
    origin	git@github.com:ministryofjustice/cloud-platform-reference-app.git (fetch)
    origin	git@github.com:ministryofjustice/cloud-platform-reference-app.git (push)
    $username	git@github.com:$username/cloud-platform-reference-app.git (fetch)
    $username	git@github.com:$username/cloud-platform-reference-app.git (push)

You now have a functioning git repository that contains a simple Django application. Have a browse around and get familiar with the directory structure. From here on push/pull operations will apply to the remote pointing to git@github.com:$username/cloud-platform-reference-app.git
...
Let's make use of the command line tool `kubectl` to browse around the cluster to see what it looks like before we deploy our application:
    ...
    tiller-deploy-64b7bd5649-fzngz   1/1       Running   0          2h

*The `<env-name>` here is the environment you created, listed in the requirements section at the beginning of the document. The Tiller pod must be present*
...
`$ helm init --client-only --tiller-namespace <env-name>`
